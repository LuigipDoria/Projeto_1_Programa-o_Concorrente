...
void* virtual_clock_run(void* arg) {
    ...
    /*
        Foi adicionada uma logica para caso o restaurante ja tenha fechado, não tenha mais nenhum cliente dentro do sushi shop e
        niguem esperando uma vaga. O virtual clock desliga. 
    */

    queue_t* queue = globals_get_queue(); // Recebe a fila global do restaurante
    ...
    while (TRUE) {
        if (self->current_time >= self->closing_time) {
            ...
            globals_set_estado_restaurante(); // Muda o estado do restaurante para fechado
            if (globals_get_estado_restaurante() == TRUE && globals_get_n_codigos_finalizados() == 4) { // Ve se o restaurante fechou e se ja foi removido todo mundo da fila
               break; // Caso já tenha fechado e não tiver niguem na fila do restaurante, acaba o programa
            }
...
virtual_clock_t* virtual_clock_init(config_t* config) {
    ...
    pthread_mutex_init(&self->_n_codigos_finalizados, NULL);
...
void virtual_clock_finalize(virtual_clock_t* self) {
    ...
    pthread_mutex_destroy(&self->_n_codigos_finalizados);
    free(self);
}
