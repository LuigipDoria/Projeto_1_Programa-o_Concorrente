...
void* queue_run(void *arg) {
    ...
    while (TRUE) {
        ...
        if (globals_get_estado_restaurante() == TRUE) { // Ve se o restaurante fechou
            break; // Caso já tenha fechado, não é colocado mais niguem na fila do restaurante
        }
    ...
    pthread_mutex_lock(&clock->_n_codigos_finalizados);
    globals_set_n_codigos_finalizados();
    pthread_mutex_unlock(&clock->_n_codigos_finalizados);
    free(self);
    pthread_exit(NULL);
}
